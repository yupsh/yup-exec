...
func TestOptions(t *testing.T) {
	cases := []struct {
		...
		{"none", []string{name}, 0},
		{"dir", []string{name, "-C", "/tmp"}, 1},
		{"shell-flag", []string{name, "--shell", "sh"}, 1},
		{"env", []string{name, "-e", "A=1", "-e", "B=2"}, 2},
		{"use-shell", []string{name, "-s"}, 1},
		{"ignore-errors", []string{name, "--ignore-errors"}, 1},
		{"quiet", []string{name, "-q"}, 1},
		{"every", []string{name, "-C", "/tmp", "--shell", "sh", "-e", "A=1", "-s", "--ignore-errors", "-q"}, 6},
	}
...
func TestBuild_PassesOperandsToExec(t *testing.T) {
	src, filter, err := build(invocation(t, name, "echo", "hi"))
